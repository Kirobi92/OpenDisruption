<script lang="ts">
  ...
  async function sendMessage(e: Event) {
    ...
    finally { loading = false; setTimeout(() => messagesEnd?.scrollIntoView({ behavior: 'smooth' }), 100); }
  }
...
<div class="flex h-[calc(100vh-3.5rem)] md:h-[calc(100vh-3.5rem)] bg-gray-950 text-white overflow-hidden">
  <!-- Sidebar -->
  <div class="
  ...
  ">
    <div class="p-4 border-b border-white/10 space-y-3">
      ...
      <button onclick={createNew} class="w-full flex items-center justify-center gap-2 px-3 py-2 bg-aurora-cyan/15 hover:bg-aurora-cyan/25 text-aurora-cyan rounded-lg text-sm transition-colors">
        <Plus class="w-4 h-4" /> Neues Gespräch
      </button>
      <div class="flex flex-wrap gap-1.5">
        {#each availableZones as zone}
          <button onclick={() => selectedZone = zone} class="rounded-full border px-2 py-0.5 text-xs transition-colors {selectedZone === zone ? ZONE_COLORS[zone] : 'border-white/10 text-gray-400'}">
            {ZONE_LABELS[zone]}
          </button>
        {/each}
      </div>
    ...
    <div class="flex-1 overflow-y-auto p-2">
      {#each conversations as conv}
        <button onclick={() => selectConversation(conv)} class="w-full text-left px-3 py-2 rounded-lg mb-1 transition-colors {activeConversation?.id === conv.id ? 'bg-aurora-cyan/15 text-aurora-cyan' : 'hover:bg-white/5 text-gray-300'}">
    ...
    <div class="p-4 border-t border-white/10">
      <a href="{base}/login" class="w-full flex items-center justify-center gap-2 px-3 py-2 bg-red-600/20 hover:bg-red-600/30 text-red-300 rounded-lg text-sm transition-colors">
  ...
  <div class="flex-1 flex flex-col min-w-0">
    <!-- Header -->
    <div class="border-b border-white/10 bg-gray-900/50 px-4 py-2 flex items-center gap-2">
      <button onclick={() => sidebarOpen = !sidebarOpen} class="md:hidden text-gray-400 hover:text-white p-1">
      ...
      {#if activeConversation}
        ...
        <span class="text-xs text-gray-400">{selectedModel || runtimeOptions.default_model}</span>
        <span class="text-xs text-gray-500">{runtimeOptions.agent_options.find(a => a.id === selectedAgent)?.label ?? selectedAgent}</span>
      {/if}
      <button onclick={() => settingsOpen = !settingsOpen} class="ml-auto flex items-center gap-1 px-2 py-1 rounded-lg border text-xs transition-colors {settingsOpen ? 'border-aurora-cyan/40 text-aurora-cyan' : 'border-white/10 text-gray-400 hover:text-white'}">
        <Settings2 class="w-3.5 h-3.5" />
      </button>
    </div>

    {#if settingsOpen}
      <div class="border-b border-white/10 bg-gray-900 px-4 py-3 grid grid-cols-2 md:grid-cols-4 gap-3">
        <label class="space-y-1">
          <span class="text-[10px] uppercase tracking-wider text-gray-500">Agent</span>
          <select bind:value={selectedAgent} class="w-full rounded-lg border border-white/10 bg-gray-950 px-2 py-1.5 text-xs text-white">
        ...
        <label class="space-y-1">
          ...
          <select bind:value={selectedModel} class="w-full rounded-lg border border-white/10 bg-gray-950 px-2 py-1.5 text-xs text-white">
        ...
        <div class="space-y-1">
          ...
          <div class="flex flex-wrap gap-1">
            {#each runtimeOptions.reasoning_modes as mode}
              <button onclick={() => reasoningMode = mode.id} class="px-2 py-1 rounded border text-[10px] transition-colors {reasoningMode === mode.id ? 'border-aurora-violet/60 bg-aurora-violet/15 text-aurora-violet' : 'border-white/10 text-gray-400'}">{mode.label}</button>
            {/each}
          </div>
        </div>
        <label class="flex items-center gap-2 text-xs text-gray-300">
          <input type="checkbox" bind:checked={showReasoning} class="rounded border-white/20" />
          Reasoning anzeigen
        </label>
      </div>
    {/if}
      ...
      {:else}
        {#each messages as msg}
          <div class="flex {msg.role === 'user' ? 'justify-end' : 'justify-start'}">
            <div class="max-w-[80%] p-3 rounded-2xl text-sm {msg.role === 'user' ? 'bg-aurora-cyan/20 text-white' : 'bg-white/5 text-gray-100'}">
    ...
    {#if activeConversation}
      <div class="border-t border-white/10 p-3">
        <form onsubmit={sendMessage} class="flex items-end gap-2">
          <button type="button" onclick={toggleVoice} disabled={transcribing} class="flex-shrink-0 p-2.5 rounded-xl transition-colors disabled:opacity-50 {recording ? 'bg-red-600 hover:bg-red-700' : 'bg-white/5 hover:bg-white/10'}">
          ...
          <textarea
            ...
            rows={1}
            class="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white resize-none focus:outline-none focus:border-aurora-cyan/40 placeholder-gray-600"
          ></textarea>
          <button type="submit" disabled={loading || !input.trim()} class="flex-shrink-0 p-2.5 bg-aurora-cyan/20 hover:bg-aurora-cyan/30 text-aurora-cyan rounded-xl transition-colors disabled:opacity-30">
            <Send class="w-5 h-5" />
          </button>
        </form>
      </div>
