...
<div class="px-4 py-6 sm:px-6 max-w-2xl mx-auto space-y-6">
  ...
  <section class="rounded-3xl border border-white/10 bg-gray-950/70 p-5 space-y-3">
    <div class="flex items-center gap-3 mb-4">
      <User class="h-5 w-5 text-violet-400" />
    ...
    <div class="space-y-2">
      {#each [['Name', user?.name ?? user?.username ?? '—'], ['E-Mail', user?.email ?? '—'], ['Zone', user?.zone ?? '—']] as [label, value]}
        <div class="flex justify-between rounded-2xl border border-white/10 bg-black/20 px-4 py-2.5">
    ...
    {#each [['Aktuelles Passwort', pwOld, (v: string) => pwOld = v], ['Neues Passwort', pwNew, (v: string) => pwNew = v], ['Bestätigen', pwConfirm, (v: string) => pwConfirm = v]] as [label, val, setter]}
      <div>
        ...
        <input type="password" value={val} oninput={e => (setter as any)((e.target as HTMLInputElement).value)}
          class="w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-2.5 text-sm text-white focus:outline-none focus:border-amber-500/50" />
      </div>
    {/each}
    {#if pwError}<div class="flex items-center gap-2 text-sm text-red-300"><XCircle class="h-4 w-4" />{pwError}</div>{/if}
    {#if pwSuccess}<div class="flex items-center gap-2 text-sm text-emerald-300"><CheckCircle class="h-4 w-4" />{pwSuccess}</div>{/if}
    <button onclick={changePassword} disabled={!pwOld || !pwNew || !pwConfirm || pwLoading}
      class="flex items-center gap-2 rounded-2xl bg-amber-600 px-5 py-2.5 text-sm font-medium text-white hover:bg-amber-500 disabled:opacity-50 transition-colors">
  ...
  <!-- Logout -->
  <section class="rounded-3xl border border-red-500/20 bg-red-500/5 p-5">
    <h2 class="text-sm font-semibold text-white mb-3">Session</h2>
    <button onclick={logout} class="flex items-center gap-2 rounded-2xl border border-red-500/30 bg-red-500/10 px-5 py-2.5 text-sm font-medium text-red-300 hover:bg-red-500/20 transition-colors">
